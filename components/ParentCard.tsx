        ...
        <div className={`absolute inset-0 w-full h-full p-5 flex flex-col ${isDeleting ? 'opacity-0 pointer-events-none' : 'opacity-100'} bg-transparent`} style={{backfaceVisibility: 'hidden'}}>
          ...
          <div className="text-sm text-gray-700 mb-4">
            <div className="flex flex-col gap-1">
              <div className="text-sm text-gray-600">{parent.phone ? <span>📞 <a href={`tel:${parent.phone.replace(/\s+/g, '')}`} className="text-blue-600 hover:underline touch-manipulation">{parent.phone}</a></span> : '—'}</div>
              <div className="text-sm text-gray-600">{parent.email ? <span>✉️ <a href={`mailto:${parent.email}`} className="text-blue-600 hover:underline touch-manipulation">{parent.email}</a></span> : null}</div>
            </div>
          </div>
            ...
            <div className="flex flex-wrap gap-2">
              {(parent.children || []).map(c => (
                <button
                  ...
                  className="px-3 py-2 bg-gray-100 text-blue-700 rounded text-sm border text-left focus:outline-none hover:bg-gray-50 cursor-pointer hover:underline touch-manipulation min-h-[44px] flex flex-col justify-center"
