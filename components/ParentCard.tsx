...
export default function ParentCard({ parent, color, parentDue, onChildClick, onEdit, onDelete, onAdjustmentSaved }: { parent: Parent; color?: string; parentDue?: number; onChildClick?: (child: { id: string; name: string; group?: string }) => void; onEdit?: (p: Parent) => void; onDelete?: (id: string) => void; onAdjustmentSaved?: () => void }) {
  ...
  return (
    <>
    <div className={`rounded-2xl shadow-md border border-border-default ${color || 'bg-card'} relative flex flex-col h-full ${hasAdjustment ? 'ring-2 ring-yellow-400' : ''}`} style={{ minHeight: 420 }}>
      {/* Face avant */}
      <div className={`flex flex-col h-full ${isDeleting ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
        ...
        <div className="flex items-center gap-3 px-5 pt-5 pb-4 border-b border-border-default dark:bg-input">
          ...
          <div className="flex-shrink-0">
            <span className="inline-flex items-center gap-1 text-xs font-semibold bg-card text-emerald-700 dark:text-emerald-300 px-2.5 py-1 rounded-full shadow-sm border border-emerald-100 dark:border-emerald-800">
              {(() => {
        ...
        {/* Infos contact */}
        <div className="px-5 py-3 space-y-1.5 border-b border-border-default">
          <div className="flex items-center gap-2 text-sm">
            <svg className={`w-3.5 h-3.5 flex-shrink-0 ${phoneVal ? 'text-[#0b5566] dark:text-accent' : 'text-muted'}`} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg>
            {phoneVal ? <a href={`tel:${phoneVal.replace(/\s+/g, '')}`} className="text-[#0b5566] dark:text-accent hover:underline truncate touch-manipulation">{phoneVal}</a> : <span className="text-muted">—</span>}
          </div>
          <div className="flex items-center gap-2 text-sm">
            <svg className={`w-3.5 h-3.5 flex-shrink-0 ${emailVal ? 'text-[#0b5566] dark:text-accent' : 'text-muted'}`} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
            {emailVal ? <a href={`mailto:${emailVal}`} className="text-[#0b5566] dark:text-accent hover:underline truncate touch-manipulation">{emailVal}</a> : <span className="text-muted">—</span>}
          </div>
          <div className="flex items-start gap-2 text-sm">
            <svg className={`w-3.5 h-3.5 flex-shrink-0 mt-0.5 ${addressVal ? 'text-[#0b5566] dark:text-accent' : 'text-muted'}`} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path strokeLinecap="round" strokeLinejoin="round" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
            <div className="leading-snug min-w-0">
              <div className="truncate text-primary">{addressVal || <span className="text-muted text-xs">—</span>}</div>
              <div className="truncate text-muted text-xs">{[postalCodeVal, cityVal].filter(Boolean).join(' ') || <span>—</span>}</div>
            </div>
          </div>
        </div>
        ...
        <div className="px-5 py-3 flex-1 overflow-auto">
          ...
          <div className="flex flex-wrap gap-2 min-h-[100px]">
            {(parent.children || []).map(c => (
              <div key={c.child.id} className="bg-card/80 rounded-xl p-2.5 border border-card shadow-sm w-[7.5rem] flex flex-col justify-between gap-1">
                <button
                  onClick={() => { if (onChildClick) { onChildClick(c.child); } else navigate(`/parent/child/${c.child.id}/reports`); }}
                  className="text-left text-sm font-semibold text-[#0b5566] dark:text-accent hover:underline truncate block"
                  aria-label={`Voir les rapports de ${c.child.name}`}>
                  <span className="truncate block">{c.child.name}</span>
                  {c.child.group ? <span className="text-xs text-muted font-normal">({c.child.group})</span> : null}
                </button>
                <div className="min-h-[18px] flex items-center">
                  {(() => {
                    ...
                    return (
                      ...
                      }} className="text-xs text-emerald-700 dark:text-emerald-300 hover:underline">{t('children.prescription.view_button')}</button>
                    );
                  })()}
                </div>
                {isCurrentParent && (
                  <div className="flex items-center gap-1">
                    ...
                    <label htmlFor={`presc-${c.child.id}`} className="text-xs bg-[#0b5566] dark:bg-accent text-white px-1.5 py-0.5 rounded-lg cursor-pointer">{t('parent.prescription.upload')}</label>
                    {prescriptions[c.child.id] && prescriptions[c.child.id].url ? (
                      <button onClick={() => deletePrescription(c.child.id)} className="text-xs bg-red-50 dark:bg-red-950 text-red-600 px-1.5 py-0.5 rounded-lg">{t('parent.prescription.delete')}</button>
                    ) : null}
                  </div>
                )}
              </div>
        ...
        <div className="px-5 py-3 bg-input border-t border-border-default">
          {(annualPerChild > 0 || centerSettings.dailyRate > 0) && (
          <div className="bg-card/60 rounded-xl px-3 py-2.5 mb-3 text-center">
            {centerSettings.dailyRate > 0 && (
              <div className="flex items-center justify-center gap-2 mb-0.5">
                <span className="text-xs text-gray-500">{t('parent.cotisation.this_month')} :</span>
                <span className="font-extrabold text-[#0b5566] dark:text-accent text-base">{(parentDue || 0)} €</span>
            ...
            {annualPerChild > 0 && <div className="text-xs text-muted mt-0.5">{t('parent.cotisation.annual_total')} : <span className="font-bold text-primary">{((parent.children?.length || 0) * annualPerChild)} €</span></div>}
          </div>
          )}
          {isAdminUser && (annualPerChild > 0 || centerSettings.dailyRate > 0) && (
            <div className="text-center mb-2">
              <button className="text-xs font-semibold text-[#0b5566] dark:text-accent hover:underline" onClick={() => setShowAdjModal(true)}>{t('adjustment.button')}</button>
            </div>
          )}
          <div className="flex justify-center gap-2">
            {onEdit && (
              <button onClick={() => { if (onEdit) onEdit(parent); }} className="flex items-center gap-1.5 px-2 py-1.5 text-secondary hover:text-[#0b5566] dark:hover:text-accent hover:bg-card transition rounded-lg text-xs font-medium touch-manipulation" title={t('children.action.edit')}>
            ...
            {onDelete && (
              <button onClick={() => setIsDeleting(true)} className="flex items-center gap-1.5 px-2 py-1.5 text-secondary hover:text-red-500 transition rounded-lg text-xs font-medium touch-manipulation hover:bg-red-50 dark:bg-red-950" title={t('children.action.delete')}>
      ...
      {isDeleting && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-card rounded-2xl shadow-xl p-8 z-10">
          <div className="w-12 h-12 bg-red-100 dark:bg-red-900 rounded-2xl flex items-center justify-center mb-3">
          ...
          <div className="text-lg font-bold mb-1 text-primary text-center">{t('modal.delete.title')}</div>
          <div className="text-sm text-secondary mb-6 text-center">{t('parent.delete.confirm_body')}</div>
          <div className="flex gap-3 w-full">
            <button onClick={() => setIsDeleting(false)} className="flex-1 border border-border-default text-primary rounded-xl px-4 py-2.5 text-sm font-medium hover:bg-input transition">{t('modal.cancel')}</button>
      ...
      {prescriptionModal.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
          <div className="bg-card rounded-2xl shadow-xl p-4 w-full max-w-2xl relative mx-4">
            <button onClick={() => setPrescriptionModal({ open: false, url: null })} className="absolute top-3 right-3 text-secondary">×</button>
            <div className="text-lg font-bold mb-2 text-primary">Ordonnance de {prescriptionModal.childName || 'cet enfant'}</div>
