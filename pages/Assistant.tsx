...
export default function Assistant() {
  ...
  function localPush(msg: Message) {
    ...
    setTimeout(() => scroller.current?.scrollTo({ top: scroller.current!.scrollHeight, behavior: 'smooth' }), 120);
  }
  ...
  return (
    <div className="relative z-0 min-h-screen bg-[#f1f5f9] p-4 md:pl-64 w-full">
      <Helmet>
        <title>{t('assistant.title', 'Assistant')}</title>
      </Helmet>
        ...
        <div className="space-y-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4 w-full">
            <div className="flex-1 min-w-0">
              <h1 className="text-3xl sm:text-4xl font-bold text-gray-900 mb-1 text-left">{t('assistant.header.title', 'Assistant IA Les frimousses')}</h1>
                ...
                <div className="flex flex-col h-[70vh] md:h-[72vh]">
                  <div ref={scroller} className="space-y-6 p-4 overflow-auto flex-1">
                    ...
                    <div className="bg-white rounded-2xl p-5 shadow-md relative overflow-visible">
                          <div className="absolute left-4 top-6 w-12 h-12 rounded-lg bg-gradient-to-br from-indigo-500 to-violet-500 flex items-center justify-center text-white text-xl font-bold" aria-hidden="true">🤖</div>
                      <div className="max-w-4xl w-full mx-auto relative">
                        ...
                        <div className="flex flex-col items-center text-center">
                          ...
                          <div className="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-3 justify-center items-stretch">
                            <div className="border rounded-lg p-3 flex flex-col sm:flex-row items-start gap-3 w-full">
                              <div className="flex-1 min-w-0">
                                <div className="font-semibold self-start">{t('assistant.option.nutrition', 'Nutrition advice')}</div>
                                <div className="text-sm text-gray-500">{t('assistant.option.nutrition.example', "What to cook for my child?")}</div>
                              </div>
                            </div>

                            <div className="border-2 border-dashed border-blue-300 rounded-lg p-3 flex flex-col sm:flex-row items-start gap-3 bg-blue-50 w-full">
                              <div className="flex-1 min-w-0">
                                <div className="font-semibold self-start">{t('assistant.option.education', 'Educational tips')}</div>
                            ...
                            <div className="border rounded-lg p-3 flex flex-col sm:flex-row items-start gap-3 w-full">
                              <div className="flex-1 min-w-0">
                                <div className="font-semibold self-start">{t('assistant.option.activities', 'Suggested activities')}</div>
                  ...
                  <div className="mt-4 pt-2 border-t border-gray-100 bg-white">
                    <form onSubmit={handleSubmit} className="flex gap-3 items-center p-3">
                      <input
                        aria-label={t('assistant.input.aria', 'Your question')}
                        className="flex-1 px-4 py-3 rounded-full border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#a9ddf2]"
                        placeholder={t('assistant.input.placeholder', "Ask the assistant...")}
                        value={message}
                        onChange={e => setMessage(e.target.value)}
                        disabled={loading}
                      />
                      <button type="submit" disabled={loading} className="px-4 py-2 rounded-full bg-indigo-600 text-white font-semibold">{loading ? t('assistant.loading', 'Loading...') : t('assistant.send.button', 'Send')}</button>
                    </form>
