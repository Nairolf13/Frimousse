...
const ParentDashboard: React.FC = () => {
  ...
  const inputCls = "border border-border-default rounded-xl px-3 py-2.5 text-sm w-full focus:outline-none focus:ring-2 focus:ring-[#0b5566]/30 focus:border-[#0b5566] bg-card";
  const labelCls = "block text-xs font-semibold text-secondary uppercase tracking-wider mb-1";
        ...
        <div className="max-w-7xl mx-auto w-full px-0 sm:px-2 md:px-4">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6 w-full">
            <div className="flex items-start gap-3">
              ...
              <div className="pt-0.5">
                ...
                <p className="text-xs sm:text-sm text-secondary mt-0.5">{t('page.parent.description')}</p>
              </div>
            </div>
            {(user && typeof user.role === 'string' && (user.role.toLowerCase() === 'admin' || user.role.toLowerCase().includes('super') || user.nannyId == null)) && (
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-2 px-3 py-2 sm:px-5 sm:py-2.5 rounded-xl bg-amber-500 hover:bg-amber-600 text-white text-xs sm:text-sm font-semibold shadow-sm transition flex-shrink-0"
          ...
          <div className="flex flex-col lg:flex-row lg:items-center gap-3 mb-6">
            <div className="grid grid-cols-3 gap-3 w-full lg:w-auto lg:flex lg:flex-row">
              {[
                { label: t('stats.total'), value: stats.parentsCount, color: 'text-[#0b5566] bg-accent-light' },
                { label: t('stats.active'), value: parents.filter(p => p.children && p.children.length > 0).length, color: 'text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950' },
                { label: t('stats.new'), value: parents.filter(p => { if (!p.createdAt) return false; const d = Math.ceil(Math.abs(new Date().getTime() - new Date(p.createdAt).getTime()) / (1000*60*60*24)); return d <= 30; }).length, color: 'text-violet-700 dark:text-violet-300 bg-violet-50 dark:bg-violet-950' },
              ].map(kpi => (
                <div key={kpi.label} className="bg-card rounded-2xl shadow-sm px-4 py-3 flex items-center gap-3 lg:min-w-[100px]">
                  <div className={`text-xl font-extrabold ${kpi.color.split(' ')[0]}`}>{kpi.value}</div>
                  <div className="text-xs text-secondary leading-tight">{kpi.label}</div>
                </div>
              ))}
            </div>
            <div className="flex flex-col sm:flex-row gap-2 lg:ml-auto">
              {user && typeof user.role === 'string' && user.role === 'super-admin' && (
                <select value={centerFilter || ''} onChange={e => setCenterFilter(e.target.value || null)} className="border border-border-default rounded-xl px-3 py-2.5 bg-card text-primary shadow-sm text-sm w-full sm:w-auto min-h-[44px] focus:outline-none focus:ring-2 focus:ring-[#0b5566]/30">
          ...
          {(adding || editingParent) && (
            <form onSubmit={async (e) => {
            ...
            }} className="mb-6 bg-card rounded-2xl shadow-sm">
              <div className="flex items-center justify-between px-5 py-4 border-b border-border-default">
                <h2 className="text-base font-bold text-primary">{editingParent ? t('parent.form.submit.save') : t('parent.add')}</h2>
                <button type="button" onClick={() => { setAdding(false); setEditingParent(null); setForm({ firstName: '', lastName: '', email: '', phone: '', password: '', confirmPassword: '', address: '', postalCode: '', city: '', region: '', country: '' }); setFormError(null); }} className="p-2 text-muted hover:text-secondary hover:bg-card-hover rounded-lg transition-colors">
              ...
              <div className="p-5 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                ...
                <div id="parent-form-address" data-tour="parent-form-address" className="relative md:col-span-2">
                  ...
                  {openAddress && placeSuggestions && placeSuggestions.length > 0 && (
                    <div className="absolute z-50 bg-card shadow-lg rounded-xl mt-1 w-full max-h-48 overflow-auto border border-border-default">
                      {placeSuggestions.map((p, i) => (
                        <div key={i} className="px-3 py-2.5 hover:bg-input cursor-pointer text-sm border-b border-border-default last:border-0" onClick={() => selectPlace(p)}>
                          <div className="font-medium text-primary">{(p.house_number ? `${p.house_number} ` : '') + (p.street || p.name || '')}</div>
                          <div className="text-xs text-muted">{[p.postcode, p.city, p.state, p.country].filter(Boolean).join(' • ')}</div>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                {form.email && user?.email && form.email.toLowerCase() === user.email.toLowerCase() && (
                  <div className="sm:col-span-2 lg:col-span-3 flex items-start gap-2 bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 rounded-xl px-4 py-3 text-sm text-blue-800 dark:text-blue-200">
                ...
                {(adding || editingParent) && !(form.email && user?.email && form.email.toLowerCase() === user.email.toLowerCase()) && (
                  <div className="md:col-span-2 lg:col-span-3">
                    <div className="text-xs font-semibold text-secondary uppercase tracking-wider mb-2">{t('parent.password.requirements.title', 'Le mot de passe doit contenir :')}</div>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                      {[
                        ...
                        <div key={r.label} className={`flex items-center gap-1.5 text-xs rounded-lg px-2 py-1.5 ${r.ok ? 'bg-emerald-50 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-300' : 'bg-red-50 dark:bg-red-950 text-red-600'}`}>
              ...
              <div className="flex items-center justify-between gap-3 px-5 py-4 border-t border-border-default">
                <div className="text-xs text-muted">{t('parent.form.required_note')} <span className="text-red-500">*</span></div>
                <div className="flex gap-2">
                  <button type="button" onClick={() => { setAdding(false); setEditingParent(null); setForm({ firstName: '', lastName: '', email: '', phone: '', password: '', confirmPassword: '', address: '', postalCode: '', city: '', region: '', country: '' }); setFormError(null); }} className="px-4 py-2 border border-border-default text-primary rounded-xl hover:bg-input transition text-sm font-medium">{t('parent.form.cancel')}</button>
                  <button type="submit" className="px-5 py-2 text-white rounded-xl transition text-sm font-semibold bg-amber-500 hover:bg-amber-600">{editingParent ? t('parent.form.submit.save') : t('parent.form.submit.add')}</button>
                </div>
              </div>
              {formError && <div className="mx-5 mb-4 text-sm text-red-600 bg-red-50 dark:bg-red-950 border border-red-100 dark:border-red-800 rounded-xl px-3 py-2">{formError}</div>}
            </form>
          )}

          {/* Bannière succès */}
          {successMessage && (
            <div className="mb-4 flex items-center gap-2 text-emerald-700 dark:text-emerald-300 font-semibold bg-emerald-50 dark:bg-emerald-950 border border-emerald-200 dark:border-emerald-800 rounded-xl px-4 py-3 text-sm">
              ...
              return parents.map((p) => {
                return (
                  <ParentCard
                    ...
                    onDelete={async (id) => {
                      try {
                        const res = await fetchWithRefresh(`api/parent/${id}`, { method: 'DELETE', credentials: 'include' });
          ...
          {adminResetModal && adminResetModal.open && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm p-4">
              <div className="bg-card rounded-2xl shadow-2xl w-full max-w-md p-6">
                <h3 className="text-lg font-bold mb-2 text-primary">Confirmer la réinitialisation du mot de passe</h3>
                <p className="text-sm text-secondary mb-5">Vous allez réinitialiser le mot de passe de ce parent. Voulez-vous continuer ?</p>
                <div className="flex gap-3">
                  <button onClick={() => { setPendingSave(null); setAdminResetModal(null); }} className="flex-1 border border-border-default text-primary rounded-xl px-4 py-2.5 text-sm font-medium hover:bg-input transition">Annuler</button>
      ...
      <div className="max-w-7xl mx-auto w-full px-0 sm:px-2 md:px-4">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-secondary">Vos informations et vos enfants</p>
          </div>
        </div>
        ...
        {parentForCard ? (
          ...
          <div className="bg-card rounded-2xl shadow-sm p-10 text-center text-muted">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {children.map(child => (
              <div key={child.id} className="bg-card rounded-2xl shadow-sm p-4 flex items-center justify-between gap-3">
                <div>
                  <button type="button" className="font-semibold text-primary hover:text-[#0b5566] text-left transition-colors" onClick={(e) => { e.stopPropagation(); setSelectedChild(child); setShowChildModal(true); }}>{child.name}</button>
                  <div className="text-xs text-muted mt-0.5">Groupe : {child.group}</div>
                </div>
                <div className="flex gap-2 flex-shrink-0">
                  <button className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-accent-light text-[#0b5566] hover:bg-accent transition" onClick={() => navigate(`/parent/child/${child.id}/schedule`)}>Planning</button>
                  <button className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-accent-light text-[#0b5566] hover:bg-accent transition" onClick={() => navigate(`/parent/child/${child.id}/reports`)}>Rapports</button>
                </div>
              </div>
            ))}
          </div>
