      ...
      <main className="flex-1 w-full">
        ...
        <section className="relative pt-32 pb-20 md:pt-40 md:pb-28 overflow-hidden bg-gradient-to-br from-brand-800 via-brand-600 to-brand-500">
          ...
          <div className="relative z-10 max-w-4xl mx-auto text-center px-6">
            <span className="inline-block text-xs font-bold uppercase tracking-widest text-white bg-white/15 px-4 py-1.5 rounded-full mb-6 border border-white/20">{t('supportpage.hero.tag', 'Support')}</span>
            <h1 className="text-4xl sm:text-5xl md:text-6xl font-extrabold !text-[#ffffff] leading-[1.1] tracking-tight mb-6">
              {t('supportpage.hero.title_line1', 'Comment pouvons-nous')}
              <br />
              <span className="bg-gradient-to-r from-brand-200 via-brand-100 to-cream-100 bg-clip-text text-transparent">{t('supportpage.hero.title_highlight', 'vous aider ?')}</span>
            </h1>
            <p className="text-lg md:text-xl !text-[#ffffff] max-w-2xl mx-auto leading-relaxed">
              {t('supportpage.hero.description', "Retrouvez toutes les ressources pour bien utiliser Frimousse, obtenir de l'aide ou contacter notre équipe.")}
            </p>
          </div>
          ...
          <div className="max-w-6xl mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="group p-8 rounded-3xl border border-gray-100 hover:border-brand-200 bg-white hover:shadow-xl hover:shadow-brand-100/50 transition-all duration-300">
                ...
                <h2 className="text-xl font-bold text-gray-900 mb-4">{t('supportpage.contact.title', 'Contactez-nous')}</h2>
                <div className="space-y-4 mb-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-xs text-gray-400 uppercase font-semibold tracking-wider">{t('supportpage.contact.email_label', 'Email')}</p>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-xs text-gray-400 uppercase font-semibold tracking-wider">{t('supportpage.contact.phone_label', 'Téléphone')}</p>
                ...
                <div className="p-4 rounded-2xl bg-amber-50 border border-amber-100">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="font-semibold text-amber-800 text-sm">{t('supportpage.premium.title', 'Support premium')}</p>
                      <p className="text-amber-700 text-xs mt-0.5">{t('supportpage.premium.description', 'Assistance prioritaire pour les abonnés Essentiel & Pro (jours ouvrés, 9h-18h).')}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="group p-8 rounded-3xl border border-gray-100 hover:border-brand-200 bg-white hover:shadow-xl hover:shadow-brand-100/50 transition-all duration-300">
                ...
                <h2 className="text-xl font-bold text-gray-900 mb-4">{t('supportpage.guides.title', 'Guides & Documentation')}</h2>
                <div className="space-y-2">
                  {[
                    { to: '/guide-demarrage', label: t('supportpage.guides.quick_start', 'Guide de démarrage rapide'), icon: '\u{1F680}' },
                    { to: '/guide-ajouter-enfant', label: t('supportpage.guides.add_child', 'Ajouter un enfant'), icon: '\u{1F476}' },
                    { to: '/guide-planning', label: t('supportpage.guides.manage_planning', 'Gérer un planning'), icon: '\u{1F4C5}' },
                    { to: '/guide-export-rapport', label: t('supportpage.guides.reports', 'Voir les rapports'), icon: '\u{1F4CA}' },
                    { to: '/guide-securite', label: t('supportpage.guides.privacy', 'Sécurité & RGPD'), icon: '\u{1F512}' },
                  ].map(guide => (
                    <Link
                      key={guide.to}
                      to={guide.to}
                ...
                <div className="mt-6 p-4 rounded-2xl bg-brand-50 border border-brand-100">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="font-semibold text-brand-800 text-sm">{t('supportpage.community.title', 'Communauté')}</p>
                      <p className="text-brand-600 text-xs mt-0.5">{t('supportpage.community.description', "Échangez avec d'autres utilisateurs et partagez vos bonnes pratiques.")}</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="mt-16 text-center">
              <h3 className="text-sm font-bold uppercase tracking-widest text-gray-400 mb-6">{t('supportpage.legal.title', 'Ressources complémentaires')}</h3>
              <div className="flex flex-wrap justify-center gap-3">
                {[
                  { to: '/confidentialite', label: t('supportpage.legal.privacy_policy', 'Politique de confidentialité') },
                  { to: '/cgu', label: t('supportpage.legal.terms', 'CGU') },
                  { to: '/mentions-legales', label: t('supportpage.legal.legal_notice', 'Mentions légales') },
                ].map(link => (
        ...
        <section className="py-24 px-6 bg-gradient-to-br from-brand-700 via-brand-600 to-brand-500 relative overflow-hidden">
          ...
          <div className="max-w-3xl mx-auto text-center relative z-10">
            <h2 className="text-3xl md:text-5xl font-extrabold !text-[#ffffff] tracking-tight mb-6">{t('supportpage.cta.title', 'Prêt à démarrer ?')}</h2>
            <p className="!text-[#ffffff] text-lg mb-10 max-w-xl mx-auto leading-relaxed">{t('supportpage.cta.description', 'Créez votre compte gratuitement et découvrez toutes les fonctionnalités de Frimousse.')}</p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
              <Link to="/register" className="group bg-white text-brand-700 px-8 py-4 rounded-2xl font-bold text-lg shadow-xl shadow-black/10 hover:shadow-2xl transition-all hover:-translate-y-0.5 inline-flex items-center gap-3">
                {t('supportpage.cta.primary', 'Essayer gratuitement')}
              ...
              <Link to="/" className="!text-[#ffffff] border-2 border-white/25 hover:border-white/50 px-8 py-4 rounded-2xl font-bold text-lg transition-all hover:bg-white/10">
                {t('supportpage.cta.secondary', "Retour à l'accueil")}
              </Link>
            </div>
          </div>
        </section>
