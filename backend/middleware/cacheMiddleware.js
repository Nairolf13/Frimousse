...
function cacheMiddleware(ttlMs = 30000) {
  return async (req, res, next) => {
    ...
    // Try redis first if the client is actually ready; otherwise fallback to in-memory cache
    if (redisCache && typeof redisCache.isReady === 'function' && redisCache.isReady()) {
      try {
