...
exports.register = async (req, res) => {
  ...
  const { password, name, role, nannyId, centerId, centerName, plan, address, region, country } = req.body;
  ...
  const userData = { email, password: hash, name, role, address: address || null, region: region || null, country: country || null };
...
exports.registerSubscribeInit = async (req, res) => {
  try {
    // normalize email
    const email = String(req.body.email || '').trim().toLowerCase();
    const { password, name, role, centerName, plan, address, region, country } = req.body;
