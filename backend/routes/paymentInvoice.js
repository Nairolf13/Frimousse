...
// POST /invoice/:id/send - send invoice PDF by email to the parent.
router.post('/invoice/:id/send', auth, async (req, res) => {
  try {
    ...
    // Authorization: admin can send any; parent users can send their own invoice
    if (!isAdmin) {
      if (user.parentId && user.parentId === ph.parentId) {
        /* parent sending to self - allowed */
      } else {
        return res.status(403).json({ message: 'Forbidden' });
      }
    }
    ...
    if (!parent || !parent.email) return res.status(400).json({ message: 'Parent email not found' });

    // generate PDF buffer using existing helper
    ...
    await sendTemplatedMail({
      ...
      // allow admins to bypass opt-out; parents respect opt-out
      bypassOptOut: !!isAdmin,
      respectOptOut: !isAdmin
    });

    return res.json({ ok: true });
  } catch (e) {
    console.error('invoice send error', e && e.message ? e.message : e);
